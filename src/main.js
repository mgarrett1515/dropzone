...
import { initLobby, teardownLobby } from './lobby.js';
...
// MENU SETUP
// ============================================================================
const settings = { bots:20, skill:0.7, view:'fp', shoulder:'r', mapSize:900, fov:75, sensitivity:1.0, volume:0.7 };
...
function loadPersistedSettings() {
  try {
    ...
    if (saved && typeof saved === 'object') {
      ...
      if (Number.isFinite(saved.fov))         settings.fov         = Math.max(60, Math.min(110, saved.fov));
      if (Number.isFinite(saved.sensitivity)) settings.sensitivity = Math.max(0.1, Math.min(3, saved.sensitivity));
...
function savePersistedSettings() {
  try {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      ...
      fov: settings.fov, sensitivity: settings.sensitivity, volume: settings.volume,
    }));
  } catch (e) {}
}
...
// Range-slider settings. Registers the input listener AND applies the persisted
// value immediately (runs after loadPersistedSettings above). `fmt` formats the
// number shown in the .val label; `onChange` runs after the setting updates —
// used by volume to push the new gain into the live audio graph.
...
bindSlider('fovSlider',    'fovVal',    'fov',         parseInt,   v => v);
bindSlider('sensSlider',   'sensVal',   'sensitivity', parseFloat, v => v.toFixed(2));
bindSlider('volumeSlider', 'volumeVal', 'volume',      parseFloat, v => Math.round(v * 100) + '%',
...
// Scratch vectors for positionCamera() and updatePlayer() — hot per-frame paths
const _camEye=new THREE.Vector3(),_camRight=new THREE.Vector3(),_camDir=new THREE.Vector3();
const _camBack=new THREE.Vector3(),_camFwd=new THREE.Vector3(),_camTmp=new THREE.Vector3();
const _Y_AXIS=new THREE.Vector3(0,1,0);
const _pForward=new THREE.Vector3(),_pRight=new THREE.Vector3(),_pMove=new THREE.Vector3(),_pNewPos=new THREE.Vector3();

// ── Combat juice: screen shake + pooled bullet-impact puffs ──────────────
// _camShake is a 0..1.2 magnitude bumped by addShake() (firing, taking hits,
// kills) and decayed every frame; positionCamera() jitters the camera by it.
let _camShake = 0;
function addShake(amt) { _camShake = Math.min(_camShake + amt, 1.2); }

// Impact puffs are a fixed pool of additive billboard sprites — no per-hit
// allocation. spawnImpact() reuses whichever sprite is closest to dead.
const _IMPACT_POOL_SIZE = PERF.impactPoolSize;
let _impactPool = null, _impactTex = null;
function _ensureImpactPool() {
  if (_impactPool) return;
  ...
  _impactTex = new THREE.CanvasTexture(c);
  _impactPool = [];
  for (let i = 0; i < _IMPACT_POOL_SIZE; i++) {
    const spr = new THREE.Sprite(new THREE.SpriteMaterial({
      map: _impactTex, transparent: true, opacity: 0,
      depthWrite: false, blending: THREE.AdditiveBlending,
    }));
    spr.visible = false;
    spr.userData.life = 0;
    scene.add(spr);
    _impactPool.push(spr);
  }
}
function spawnImpact(x, y, z, flesh) {
  _ensureImpactPool();
  let spr = _impactPool[0];
  for (const s of _impactPool) { if (s.userData.life < spr.userData.life) spr = s; }
  spr.position.set(x, y, z);
  spr.material.color.setHex(flesh ? 0xff4433 : 0xffdd88);
  spr.material.opacity = 0.9;
  spr.scale.setScalar(flesh ? 0.5 : 0.35);
  spr.userData.life = 1;
  spr.visible = true;
}
function _updateImpacts(dt) {
  if (!_impactPool) return;
  for (const s of _impactPool) {
    if (s.userData.life <= 0) continue;
    s.userData.life -= dt * 6;   // ~0.17s lifetime
    if (s.userData.life <= 0) { s.visible = false; s.material.opacity = 0; continue; }
    s.material.opacity = s.userData.life * 0.9;
    s.scale.addScalar(dt * 2.5);
  }
}
...
function _launchWithErrorCatch(fn) {
  ...
  teardownLobby();  // free the preview renderer before the game's renderer spins up
...
// sizeMul scales the per-house target footprint for that specific model (default 1).
const IMPORTED_RESIDENTIAL_SPECS = [
  { name: 'casa_japo',    kind: 'glb' },
  { name: 'Forge',        kind: 'glb', sizeMul: 2 },
  { name: 'objHouyse',    kind: 'obj', dir: 'obj_house' },
  { name: 'Cottage_FREE', kind: 'obj', dir: 'cottage' },
  { name: 'Wood_house',   kind: 'obj', dir: 'wood_house' },
];
...
function preloadImportedResidential() {
  return Promise.all(IMPORTED_RESIDENTIAL_SPECS.map(s => {
    const loaded = s.kind === 'glb'
      ? _loadGlb(s.name, '/imported_residential/', IMPORTED_RESIDENTIAL_MODELS)
      : _loadObjWithMtl(s.name, `/imported_residential/${s.dir}/`, IMPORTED_RESIDENTIAL_MODELS);
    // Stamp the per-model size multiplier onto the loaded template so the POI
    // spawner can read it back without needing the model's name.
    return loaded.then(() => {
      const m = IMPORTED_RESIDENTIAL_MODELS[s.name];
      if (m) m.userData.sizeMul = s.sizeMul || 1;
    });
  }));
}
...
// Measures a model's collision footprint from its own geometry. Imported
// building models usually pack the house inside a wider base/yard, and add
// roof eaves / trim / porch posts that overhang the actual walls. So:
//   1. sample only the lower-wall height band (18–55% of height) — above any
//      base slab, below the roofline;
//   2. within that band, take the 5th–95th percentile XZ extent, not the
//      absolute min/max — sparse decorative outliers get trimmed, the dense
//      cluster of real wall verts is what's kept.
// Cached on the model template (computed once, not per instance). Returns
// model-space { w, d } (same units as Box3.setFromObject).
function _computeTightFootprint(model) {
  if (model.userData._tightFP) return model.userData._tightFP;
  model.updateMatrixWorld(true);
  const full = new THREE.Box3().setFromObject(model);
  const fsize = new THREE.Vector3(); full.getSize(fsize);
  const yLo = full.min.y + fsize.y * 0.18;
  const yHi = full.min.y + fsize.y * 0.55;
  const xs = [], zs = [];
  ...
  if (xs.length > 20) {
    xs.sort((a, b) => a - b);
    zs.sort((a, b) => a - b);
    const pctExtent = arr => arr[Math.ceil(arr.length * 0.95) - 1] - arr[Math.floor(arr.length * 0.05)];
    // Clamp to [25%, 100%] of the full box — a safety net against degenerate
    // models (e.g. all geometry at one height) producing a runaway/tiny box.
    fp = {
      w: THREE.MathUtils.clamp(pctExtent(xs), fsize.x * 0.25, fsize.x),
      d: THREE.MathUtils.clamp(pctExtent(zs), fsize.z * 0.25, fsize.z),
    };
  } else {
    fp = { w: fsize.x, d: fsize.z };  // too few verts to trust — use full box
  }
  model.userData._tightFP = fp;
  return fp;
}
...
function _spawnImportedBuilding(model, x, z, rotY, targetMaxXZ) {
  ...
  scene.add(wrapper);
  // World-space wall sentinels — 4 solid perimeter walls per imported building.
  // `solid` (not `losOnly`) means they block player/bot movement as well as
  // sight + bullets, so buildings read as real obstacles. Imported models have
  // no door cutouts, so buildings are exterior cover, not enterable — loot
  // boxes spawn outside them (see spawnHouseLoot).
  const scaledW = size.x * scale, scaledD = size.z * scale, scaledH = size.y * scale;
  const cosR = Math.cos(rotY || 0), sinR = Math.sin(rotY || 0);
  const WT = 0.35;
  // Collision footprint is measured per-model from the building's actual
  // mid-height geometry (see _computeTightFootprint), not the padded full
  // bounding box — so each building file gets walls that hug its own shape.
  const _tfp = _computeTightFootprint(model);
  const colW = _tfp.w * scale, colD = _tfp.d * scale;
  ...
  _ws(-colW/2, 0, WT * 2, colD + WT);
  _ws( colW/2, 0, WT * 2, colD + WT);
  _ws(0, -colD/2, colW + WT, WT * 2);
  _ws(0,  colD/2, colW + WT, WT * 2);
  return wrapper;
}
...
// Lobby character preview + tab/play-dock wiring. buildSoldierMesh is passed in
// so lobby.js doesn't need to import from main.js (avoids a circular import).
initLobby({ buildSoldierMesh });
...
async function startGame() {
  ...
  renderer = new THREE.WebGLRenderer({ antialias: PERF.antialias, powerPreference:'high-performance' });
...
function buildWorld() {
  ...
  // One loot box per building (35% of buildings have none). The box is placed
  // just OUTSIDE one of the building's four walls — imported buildings are
  // solid (see _spawnImportedBuilding), so loot can't sit inside them. Opening
  // the box (hold F) drops its contents on the ground; contents are filled by
  // the searchableObjects population pass after buildWorld().
  function spawnHouseLoot(hx, hz, fp) {
    if (Math.random() < 0.35) return;
    const w = fp ? fp.w : 12, d = fp ? fp.d : 12;
    const off = Math.max(w, d) * 0.5 + 1.3;       // clear of the building wall
    const side = Math.floor(Math.random() * 4);
    let bx = hx, bz = hz;
    if      (side === 0) { bx -= off; bz += (Math.random() - 0.5) * d * 0.7; }
    else if (side === 1) { bx += off; bz += (Math.random() - 0.5) * d * 0.7; }
    else if (side === 2) { bz -= off; bx += (Math.random() - 0.5) * w * 0.7; }
    else                 { bz += off; bx += (Math.random() - 0.5) * w * 0.7; }
    makeLootBox(bx, bz);
  }
  ...
  for (const poi of poiSpots) {
    ...
    for (let attempts = 0; attempts < TARGET_BUILDINGS * 4 && placedCenters.length < TARGET_BUILDINGS; attempts++) {
      ...
      const sizeMul = model?.userData.sizeMul ?? 1;
      const targetSize = 21 * scaleFrac * sizeMul;
  ...
  const forestCount = Math.max(2, Math.floor((20 + Math.floor(Math.random() * 10)) * PERF.vegMul));
  ...
  const soloTrees = Math.floor(MAP.size * 0.30 * PERF.vegMul);
  ...
  const bushCount = Math.floor(MAP.size * 0.50 * PERF.vegMul);
...
// ----- Loot Box -----
// A crate the player opens (hold F) to dump its contents onto the ground,
// instead of opening the search UI. This is the primary loot source in POIs and
// the open map. Not solid (you walk up to it); contents are filled by the
// searchableObjects population pass that runs after buildWorld().
function makeLootBox(x, z) {
  const gy = sampleTerrainHeight(x, z);
  const parts = [];
  parts.push({ geo: makeBoxGeo(0.78, 0.56, 0.78, 0, 0.28, 0), color: 0x6b4a2a }); // wood body
  for (const sx of [-1, 1]) for (const sz of [-1, 1]) {                            // metal corner posts
    parts.push({ geo: makeBoxGeo(0.11, 0.60, 0.11, sx * 0.34, 0.30, sz * 0.34), color: 0x33333a });
  }
  if (!makeLootBox._mat) {
    makeLootBox._mat = new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.85 });
  }
  const crateMesh = makeMergedMesh(parts, makeLootBox._mat);
  for (const p of parts) p.geo.dispose();
  if (!crateMesh) return;
  const grp = new THREE.Group();
  grp.add(crateMesh);
  // emissive accent band so the box is spottable from a distance
  const band = new THREE.Mesh(
    new THREE.BoxGeometry(0.84, 0.10, 0.84),
  ...
  scene.add(grp);
  registerSearchable(grp, 'loot_box', x, z);
}
...
// Builds the procedural soldier silhouette used by bots and by the lobby
// character preview. Returns the part references the caller wires up — does
// NOT add the group to any scene or set a position; that's the caller's job.
export function buildSoldierMesh() {
  ...
  botGroup.add(gunGroup);

  return { group: botGroup, bodyMesh: staticMesh, headMesh: head, hipL, hipR, shoulderL, shoulderR, gunMesh: gunGroup };
}

// Bot personality archetypes — picked per-bot at spawn. Each colours how the
// bot fights: preferred engagement distance, move speed, how hard it pushes,
// and aim discipline. `weight` is the spawn-roll probability.
//   idealDist  — metres it tries to hold from its target
//   moveMul    — multiplier on patrol/engage/investigate move speed
//   aggression — scales fireChance and the "head toward the zone" patrol bias
//   aimMul     — multiplier on aimError (lower = more accurate)
//   burstMul   — multiplier on burst length before a pause
const BOT_ARCHETYPES = {
  balanced: { weight: 40, idealDist: 25, moveMul: 1.00, aggression: 1.00, aimMul: 1.00, burstMul: 1.0 },
  rusher:   { weight: 25, idealDist: 10, moveMul: 1.30, aggression: 1.30, aimMul: 1.20, burstMul: 1.4 },
  camper:   { weight: 20, idealDist: 30, moveMul: 0.55, aggression: 0.70, aimMul: 0.80, burstMul: 0.9 },
  sniper:   { weight: 15, idealDist: 65, moveMul: 0.80, aggression: 0.85, aimMul: 0.65, burstMul: 0.7 },
};
function _pickArchetype() {
  const keys = Object.keys(BOT_ARCHETYPES);
  let total = 0;
  for (const k of keys) total += BOT_ARCHETYPES[k].weight;
  let roll = Math.random() * total;
  for (const k of keys) { roll -= BOT_ARCHETYPES[k].weight; if (roll <= 0) return k; }
  return 'balanced';
}
...
class Entity {
  constructor(x, z, isPlayer=false) {
    ...
    if (isPlayer) {
      ...
      // Procedural soldier silhouette — shared with the lobby character preview.
      const soldier = buildSoldierMesh();
      this.mesh = soldier.group;
      this.mesh.position.copy(this.pos);
      this.bodyMesh = soldier.bodyMesh;
      this.headMesh = soldier.headMesh;
      this.hipL = soldier.hipL; this.hipR = soldier.hipR;
      this.shoulderL = soldier.shoulderL; this.shoulderR = soldier.shoulderR;
      this.walkPhase = Math.random() * Math.PI * 2;
      this.gunMesh = soldier.gunMesh;
      scene.add(this.mesh);
      ...
      // Personality archetype — layered on top of the difficulty scaling above.
      this.archetype = _pickArchetype();
      const _arch = BOT_ARCHETYPES[this.archetype];
      this.archIdealDist  = _arch.idealDist;
      this.archMoveMul    = _arch.moveMul;
      this.archAggression = _arch.aggression;
      this.aimError   *= _arch.aimMul;
      this.fireChance  = Math.min(0.98, this.fireChance * _arch.aggression);
      this.burstLimit  = Math.max(2, Math.round(this.burstLimit * _arch.burstMul));
      this.lastSeen = null;
      this.alertTimer = 0;
      ...
      // Snipers favour a long gun over their rolled weapon.
      if (this.archetype === 'sniper' && Math.random() < 0.6) wk = Math.random() < 0.5 ? 'sr' : 'barrett';
      this.giveWeapon(wk);
  ...
  shoot(now) {
    ...
    if (this.isPlayer) {
      ...
      addShake(stats.recoil * 0.01 * crouchRecoilMul);
    }
  ...
  die(killer) {
    ...
    if (this.isPlayer) {
      ...
      if (killer && killer.isPlayer) {
        ...
        showKillBanner(this.botName || 'BOT');
        addShake(0.12);
      } else if (killer) {
        const dist = Math.round(killer.pos.distanceTo(this.pos));
...
function generateContainerLoot(containerType) {
  const pools = {
    ...
    loot_box:     [{type:'weapon',w:5},{type:'ammo',w:3},{type:'heal',w:3},{type:'armor',w:2},{type:'attachment',w:2}],
  };
...
function makeRandomItem(type, containerType) {
  if (type==='weapon') {
    ...
    } else if (containerType === 'loot_box') {
      // POI / open-map crates — broad spread including melee
      const keys = ['pistol','deagle','smg','p90','ar','ak','shotgun','spas','sr','bat','crowbar'];
...
function _makeGltfPickupMesh(model, rx, ry, rz, targetSize) {
  ...
  clone.traverse(c => { if (c.isMesh) { c.castShadow = false; c.receiveShadow = false; const mats=Array.isArray(c.material)?c.material:[c.material]; mats.forEach(m=>{ if(m) m.side=THREE.DoubleSide; }); } });
...
function createLootMesh(item, x, z, explicitY) {
  ...
  // sampleTerrainHeight already raises to ground-flagged bboxes (sidewalks, car
  // roofs) and ignores solid walls, so loot lands on surfaces, not roof height.
  const gy = explicitY !== undefined ? explicitY : sampleTerrainHeight(x, z);
...
// ── Supply drops — periodic high-tier loot caches that pull players together ──
// A drop = a crate + a tall glowing beam (the map marker) + a cluster of
// high-tier loot around it. Bots are biased to path toward active drops, so
// they double as a contested objective between zone shrinks.
const _supplyDrops = [];          // { x, z, beam, beamLife }
let _supplyDropTimer = 40;        // seconds to the first drop
let _supplyDropsSpawned = 0;
const _MAX_SUPPLY_DROPS = 4;

function spawnSupplyDrop(x, z) {
  const gy = sampleTerrainHeight(x, z);

  // Crate — dark box with a glowing cyan trim band.
  const crate = new THREE.Group();
  const box = new THREE.Mesh(
    new THREE.BoxGeometry(1.3, 1.0, 1.3),
    new THREE.MeshStandardMaterial({ color: 0x2a3540, emissive: 0x114455, emissiveIntensity: 0.5, roughness: 0.6, metalness: 0.3 })
  );
  box.position.y = 0.5;
  crate.add(box);
  const trim = new THREE.Mesh(
    new THREE.BoxGeometry(1.36, 0.14, 1.36),
    new THREE.MeshBasicMaterial({ color: 0x33ddff })
  );
  trim.position.y = 0.5;
  crate.add(trim);
  crate.position.set(x, gy, z);
  scene.add(crate);

  // Beam — tall additive cylinder, visible from across the map.
  const beam = new THREE.Mesh(
    new THREE.CylinderGeometry(0.7, 0.7, 70, 12, 1, true),
    new THREE.MeshBasicMaterial({ color: 0x33ddff, transparent: true, opacity: 0.3, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide })
  );
  beam.position.set(x, gy + 35, z);
  beam.frustumCulled = false;
  scene.add(beam);

  // High-tier loot clustered around the crate.
  const weaponPool = ['ar', 'ak', 'sr', 'barrett', 'p90', 'spas'];
  const attachPool = ['scope4x', 'grip', 'extmag', 'comp', 'silencer'];
  const wk = weaponPool[Math.floor(Math.random() * weaponPool.length)];
  const ak = attachPool[Math.floor(Math.random() * attachPool.length)];
  createLootMesh({ type: 'weapon', key: wk, name: WEAPONS[wk].name }, x + 1.4, z);
  createLootMesh({ type: 'attachment', key: ak, name: ATTACHMENTS[ak].name }, x - 1.4, z);
  createLootMesh({ type: 'ammo', amount: 60, name: 'AMMO' }, x, z - 1.4);
  if (Math.random() < 0.5) createLootMesh({ type: 'armor', amount: 100, name: 'ARMOR' }, x, z + 1.4);
  else createLootMesh({ type: 'heal', key: 'medkit', name: 'MEDKIT' }, x, z + 1.4);

  _supplyDrops.push({ x, z, beam, beamLife: 1 });
  _supplyDropsSpawned++;
  showSupplyNotice();
}
...
function updateZone(dt) {
  ...
  // Periodic supply drops — skipped in the shooting range.
  if (!globalThis._rangeMode && _supplyDropsSpawned < _MAX_SUPPLY_DROPS) {
    _supplyDropTimer -= dt;
    if (_supplyDropTimer <= 0) {
      const ang = Math.random() * Math.PI * 2;
      const r = zone.radius * 0.7 * Math.random();
      spawnSupplyDrop(zone.center.x + Math.cos(ang) * r, zone.center.z + Math.sin(ang) * r);
      _supplyDropTimer = 70 + Math.random() * 25;
    }
  }
}
...
function updateBullets(dt) {
  for (let i=bullets.length-1; i>=0; i--) {
    ...
    if (hit) {
      ...
      if (b.owner && b.owner.isPlayer) showHitmarker(isHead);
      spawnImpact(_sv0.x, _sv0.y, _sv0.z, true);
      scene.remove(b.mesh); bullets.splice(i, 1); continue;
    }
    if (worldHit || b.traveled > b.range) {
      if (worldHit) spawnImpact(_sv0.x, _sv0.y, _sv0.z, false);
      scene.remove(b.mesh); bullets.splice(i, 1); continue;
    }
...
let _groundBlds = [];  // pre-filtered ground-flagged buildings (sidewalks, car roofs) — for sampleTerrainHeight hot path
...
function _buildCollisionCache() {
  ...
  _groundBlds = buildings.filter(b => b.userData.bbox && b.userData.ground);
...
function buildAR(group, attach) {
  if (M4_MODEL) {
    ...
    const scaleFactor = 1.35 / naturalLen;
...
function buildAK(group, attach) {
  if (AK_MODEL) {
    ...
    bbox0.getSize(size0);
    const naturalLen = Math.max(size0.x, size0.y, size0.z);
    ak.scale.setScalar(1.35 / naturalLen);
...
function updatePlayer(dt) {
  ...
  // movement vectors
  _pForward.set(Math.sin(player.yaw), 0, Math.cos(player.yaw));
  _pRight.set(Math.sin(player.yaw - Math.PI/2), 0, Math.cos(player.yaw - Math.PI/2));
  const move = _pMove.set(0, 0, 0);
  if (input.KeyW) move.add(_pForward);
  if (input.KeyS) move.sub(_pForward);
  if (input.KeyA) move.sub(_pRight);
  if (input.KeyD) move.add(_pRight);
  ...
  _pNewPos.copy(player.pos).addScaledVector(move, speed * dt);
  if (!collidesPos(_pNewPos.x, player.pos.z, 0.4)) player.pos.x = _pNewPos.x;
  if (!collidesPos(player.pos.x, _pNewPos.z, 0.4)) player.pos.z = _pNewPos.z;
...
function positionCamera() {
  ...
  _camEye.set(player.pos.x, player.pos.y - 1 + headY, player.pos.z);
  _camRight.set(Math.sin(player.yaw - Math.PI/2), 0, Math.cos(player.yaw - Math.PI/2));
  // lean offset (perp to forward)
  _camTmp.copy(_camRight).multiplyScalar(player.lean * 0.6);
  _camEye.add(_camTmp);
  const tiltZ = -player.lean * 0.12;

  if (settings.view === 'fp') {
    camera.position.copy(_camEye);
    _camDir.set(
      Math.sin(player.yaw)*Math.cos(player.pitch),
      Math.sin(player.pitch),
      Math.cos(player.yaw)*Math.cos(player.pitch)
    );
    _camTmp.copy(_camEye).add(_camDir);
    camera.up.set(Math.sin(tiltZ), Math.cos(tiltZ), 0).applyAxisAngle(_Y_AXIS, player.yaw);
    camera.lookAt(_camTmp);
    let fov = settings.fov;
    if (player.isAds) {
      const stats = player.effectiveStats();
      fov = settings.fov / stats.adsZoom;
    }
    camera.fov = THREE.MathUtils.lerp(camera.fov, fov, 0.25);
    camera.updateProjectionMatrix();
  } else {
    // third person — over the shoulder
    _camBack.set(-Math.sin(player.yaw)*Math.cos(player.pitch), -Math.sin(player.pitch), -Math.cos(player.yaw)*Math.cos(player.pitch));
    _camTmp.copy(_camRight).multiplyScalar(settings.shoulder==='r' ? 0.9 : -0.9);
    camera.position.copy(_camEye).add(_camTmp).addScaledVector(_camBack, 2.6);
    ...
    camera.up.set(Math.sin(tiltZ), Math.cos(tiltZ), 0).applyAxisAngle(_Y_AXIS, player.yaw);
    _camTmp.copy(_camEye).addScaledVector(_camFwd, 50);
    camera.lookAt(_camTmp);
    let fov = settings.fov;
    if (player.isAds) {
      const stats = player.effectiveStats();
      fov = settings.fov / Math.min(stats.adsZoom, 2.5);
    }
    camera.fov = THREE.MathUtils.lerp(camera.fov, fov, 0.25);
    camera.updateProjectionMatrix();
  }
  // Screen shake — jitter the final camera position (both view modes).
  if (_camShake > 0.0001) {
    camera.position.x += (Math.random() - 0.5) * _camShake * 0.4;
    camera.position.y += (Math.random() - 0.5) * _camShake * 0.4;
    camera.position.z += (Math.random() - 0.5) * _camShake * 0.4;
  }
}
...
function updateBots(dt) {
  ...
  for (let _bIdx = 0; _bIdx < entities.length; _bIdx++) {
    ...
    if (_tbDist2 > PERF.botNearD2 && (updateBots._frame + _bIdx) % 4 !== 0) {
    ...
    if (target) {
      ...
      // move (kite / approach)
      // Engagement range comes from the bot's archetype; a shotgun still forces
      // a close hold so shotgunners don't try to duel at sniper distance.
      const idealDist = b.weapon === 'shotgun' ? Math.min(14, b.archIdealDist) : b.archIdealDist;
      const moveDir = _sv0;
      ...
      const moveSpeed = 4.5 * settings.skill * 1.4 * b.archMoveMul;
      ...
      if (b.aiState === 'investigate' && b.investigateTimer > 0 && b.lastSeen) {
        ...
        if (d > 3) {
          ...
          const _bNpI = _sv1.copy(b.pos).addScaledVector(_sv2.set(dx,0,dz).normalize(), 4*dt*b.archMoveMul);
        ...
        if (outsideNextZone || b.aiTimer <= 0 || b.pos.distanceTo(b.aiTarget) < 4) {
          ...
          // Aggressive archetypes drift toward the zone (where fights happen)
          // more than cautious ones; storm escape still always overrides.
          const zoneBias = outsideNextZone ? 1.0 : Math.min(1, (isShrinking ? 0.9 : 0.6) * b.archAggression);
          ...
          // Active supply drops pull bots in — a contested objective. Storm
          // escape (outsideNextZone) still takes priority over chasing loot.
          if (!outsideNextZone && _supplyDrops.length && Math.random() < 0.4) {
            let nd = null, ndd = Infinity;
            for (const sd of _supplyDrops) {
              const ddx = sd.x - b.pos.x, ddz = sd.z - b.pos.z;
              const d2 = ddx*ddx + ddz*ddz;
              if (d2 < ndd) { ndd = d2; nd = sd; }
            }
            if (nd) b.aiTarget.set(nd.x, 0, nd.z);
          }
        }
...
function sampleTerrainHeight(x, z) {
  ...
  // Stand on top of ground-flagged bboxes (car roofs, sidewalks). _groundBlds is
  // pre-filtered in _buildCollisionCache — this runs dozens of times per frame.
  for (let i = 0; i < _groundBlds.length; i++) {
    const bb = _groundBlds[i].userData.bbox;
    if (x > bb.min.x && x < bb.max.x && z > bb.min.z && z < bb.max.z) {
      if (bb.max.y > h) h = bb.max.y;
    }
  }
  return h;
}
...
function hasLineOfSight(a, b) {
  // Raycast against building bboxes via the _bldGrid spatial hash. Hot path —
  // every bot queries this several times per second.
  ...
  for (let i = 1; i < steps; i++) {
    ...
    const bucket = _bldGrid.get(_gridKey(_cellOf(px), _cellOf(pz)));
    if (!bucket) continue;
    _losCheckVec.set(px, py, pz);
    for (let bi = 0; bi < bucket.length; bi++) {
      const bd = _bulletBlds[bucket[bi]];
      if (bd.userData.bbox.containsPoint(_losCheckVec)) return false;
    }
  }
  return true;
}
...
function updateLootPrompt() {
  ...
  if (container) {
    el.style.display = 'block';
    const labels = { dumpster:'SEARCH DUMPSTER', car:'SEARCH CAR', bush:'SEARCH BUSH', garbage_can:'SEARCH GARBAGE', toolbox:'SEARCH TOOLBOX', loot_box:'OPEN LOOT BOX' };
...
function startSearchProgress(container) {
  ...
  // Loot boxes are the primary loot source — open fast; other containers slower.
  const dur = container.type === 'loot_box' ? 1000 : 2250;
  ...
  const labels = { dumpster:'SEARCHING DUMPSTER', car:'SEARCHING CAR', bush:'SEARCHING BUSH', garbage_can:'SEARCHING GARBAGE', toolbox:'SEARCHING TOOLBOX', loot_box:'OPENING LOOT BOX' };
  ...
  function tick() {
    ...
    if (pct < 1) {
      ...
      if (container.type === 'garbage_can' || container.type === 'loot_box') {
...
function showHitmarker(isHeadshot) {
  ...
  h.classList.toggle('headshot', !!isHeadshot);
  if (_hitmarkerTimer) clearTimeout(_hitmarkerTimer);
  _hitmarkerTimer = setTimeout(() => { h.classList.remove('show'); }, 140);
}
let _killBannerTimer = null;
function showKillBanner(name) {
  const el = document.getElementById('killBanner');
  if (!el) return;
  el.textContent = 'ELIMINATED ' + name;
  el.classList.add('show');
  if (_killBannerTimer) clearTimeout(_killBannerTimer);
  _killBannerTimer = setTimeout(() => el.classList.remove('show'), 1400);
}
let _supplyNoticeTimer = null;
function showSupplyNotice() {
  const el = document.getElementById('supplyNotice');
  if (!el) return;
  el.classList.add('show');
  if (_supplyNoticeTimer) clearTimeout(_supplyNoticeTimer);
  _supplyNoticeTimer = setTimeout(() => el.classList.remove('show'), 3200);
}
...
// Every sound routes its final node through _audioOut() instead of
// audioCtx.destination, so the VOLUME slider can scale everything at once via
// masterGain. Lazily creates the context + gain node on first use.
function _audioOut() {
...
function playFootstep(vol = 0.12) {
  try {
    ...
    src.connect(lp); lp.connect(g); g.connect(_audioOut());
    ...
    src2.connect(hp); hp.connect(g2); g2.connect(_audioOut());
...
function playSound(type, pos, weaponKey, silenced = false) {
  try {
    ...
    if (type === 'shoot') {
      if (silenced) {
        ...
        if (wk2 === 'sr') {
          ...
          if (_audioBuffers[srSoundKey]) {
            ...
            sniperSrc.connect(lpFilter); lpFilter.connect(sniperG); sniperG.connect(_audioOut());
            ...
            sSrc.connect(sLP); sLP.connect(sG); sG.connect(_audioOut());
      ...
      o.connect(g); g.connect(_audioOut());
      ...
      imp.connect(impLP); impLP.connect(impG); impG.connect(_audioOut());
...
function animate() {
  ...
  updateZone(dt);
  _updateImpacts(dt);
  if (_camShake > 0.0001) _camShake *= 0.86; else _camShake = 0;
  // Supply drop beams — pulse, slow-rotate, fade out after ~55s.
  for (const sd of _supplyDrops) {
    if (!sd.beam) continue;
    sd.beamLife -= dt * 0.018;
    if (sd.beamLife <= 0) {
      scene.remove(sd.beam);
      sd.beam.geometry.dispose(); sd.beam.material.dispose();
      sd.beam = null;
      continue;
    }
    const pulse = 0.22 + 0.13 * Math.sin(performance.now() / 300);
    sd.beam.material.opacity = pulse * Math.min(1, sd.beamLife * 3);
    sd.beam.rotation.y += dt * 0.3;
  }
